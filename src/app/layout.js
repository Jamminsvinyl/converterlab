...
export default function Home() {
  ...
  const calculateLogic = (type, val) => {
    ...
    switch(type) {
      ...
      case 'bpm':
        ...
        break;
      case 'freq': // GELİŞTİRİLMİŞ FREKANS MOTORU
        const hz = parseFloat(inputVal);
        if(!hz || hz <= 0) return;
        const midi = 12 * (Math.log2(hz / 440)) + 69;
        const roundedMidi = Math.round(midi);
        const noteName = keys[roundedMidi % 12];
        ...
        setOutput(
          `Detected Note: ${noteName}${octave}\n` +
          `MIDI Number: ${roundedMidi}\n` +
          `Perfect Pitch: ${exactFreq.toFixed(2)} Hz\n` +
          `Detune: ${cents > 0 ? '+' : ''}${cents} cents\n\n` +
          `*If detune is 0, your sound is perfectly in tune.`
        );
        break;
      default: break;
    }
  ...
  const toolData = {
    ...
    "stats-calc": { 
        ...
        how: "Paste numbers separated by commas or spaces. (e.g., 10, 20.5, 30)", 
        why: "Mean shows the average, Median shows the exact middle, and Std Dev shows how much your data fluctuates." 
    },
    ...
    "freq-note": { 
        name: "Freq to Note Analyzer", 
        how: "Enter any frequency in Hz (e.g., 440, 55, 120.5).", 
        why: "Essential for tuning 808 kick drums, finding room resonances, or checking if an analog synth is drifting out of tune (shown in cents)." 
    },
